...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    const availabilityQuery = `
      SELECT COALESCE(SUM(pr.tempopreparo * ip.quantidade), 0) AS minutos_totais
      FROM Pedidos p
      JOIN ItensPedido ip ON p.idpedido = ip.idpedido
      JOIN Produtos pr ON ip.idproduto = pr.idproduto
    ...
    for (const item of items) {
      const itemQuery = `
        INSERT INTO itenspedido (idpedido, idproduto, quantidade, precounitario, descricaopersonalizada, idtamanho)
        VALUES ($1, $2, $3, $4, $5, $6);
      `;
      const itemValues = [newPedidoId, item.idproduto, item.quantity, item.preco, item.descricao, item.idtamanho || null];
      await client.query(itemQuery, itemValues);
    }

    await client.query('COMMIT');

    const emailSubject = `Confirmação do seu Pedido`;
    const emailHtml = `
...
const getCakeSizes = async (req, res) => {
  try {
...
const getCakeDoughs = async (req, res) => {
  try {
...
const getCakeFillings = async (req, res) => {
  try {
...
const getCakeFrostings = async (req, res) => {
  try {
